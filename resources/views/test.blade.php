...
<body>
    ...
    @foreach ($RPS->Dosen1()->get() as $Dosen)
        <h4>{{ $RPS->kodeRPS }} ada {{ $Dosen->namaDosen }} sebagai
            {{ $Detail_Peran_Dosen->where('nip', '=', $Dosen->nip)->where('kodeRPS', '=', $RPS->kodeRPS)->first()->peranDosen }}
        </h4>
    @endforeach
    ...
    @foreach ($Detail_RPS->where('kodeMingguRPS', '=', $MingguRPS->kodeMingguRPS)->all() as $Teknik_Penilaian)
        <h4>{{ $MingguRPS->kodeMingguRPS }} pake teknik penilaian
            {{ $Teknik_Penilaian->first()->Teknik_Penilaian->teknikPenilaian }}</h4>
    @endforeach
    ...
    @foreach ($Mahasiswa->Nilai_Mahasiswa()->get() as $nilai)
        <h4>{{ $nilai->kodeKelas }} dari
            {{ $Semua_Kelas->where('kodeKelas', '=', $nilai->kodeKelas)->first()->Mata_Kuliah->namaMK }} </h4>
    @endforeach
