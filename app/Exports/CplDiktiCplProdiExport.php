...
class CplDiktiCplProdiExport implements FromCollection, WithHeadings, WithColumnWidths, WithStyles
{
    protected $cpl_dikti;
    protected $cpl_prodi;
    protected $pemetaan_cpldikti_cplprodi;

    public function __construct($cpl_dikti, $cpl_prodi, $pemetaan_cpldikti_cplprodi)
    {
    ...
    public function collection(): Collection
    {
        ...
        foreach ($this->cpl_dikti as $cpl) {
            ...
            array_push($data_sementara, $cpl->kode_cpldikti);
            array_push($data_sementara, $cpl->deskripsi);
            foreach ($this->cpl_prodi as $pl) {
                if ($this->pemetaan_cpldikti_cplprodi->where('kode_cpldikti', $cpl->kode_cpldikti)->where('kode_cplprodi', $pl->kode_cplprodi)->count()) {
    ...
    public function headings(): array
    {
        ...
        foreach ($this->cpl_prodi as $pl) {
            array_push($headers, $pl->kode_cplprodi);
        }
        return $headers;
    }
