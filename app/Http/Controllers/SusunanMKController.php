...
class SusunanMKController extends Controller
{
    ...
    public function update(Request $request, Mata_Kuliah $Mata_Kuliah)
    {
        ...
        return redirect(
            url("/dashboard/kurikulum/pemetaan/susunan-mata-kuliah/")
        )->with("success", "Berhasil menyimpan perubahan!!");
    }
        ...
        return Excel::download(
                ...
                WithStyles
            {
                ...
                public function collection()
                {
                    ...
                    return $this->data->map(function ($item) use (
                        $semesterHeadings
                    ) {
                        $kategoriMK = "";
                        ...
                        $semesterCheck = in_array(
                            $item->semester,
                            $semesterHeadings
                        )
                            ? $item->semester
                            : ""; // check if the semester value exists in the semester headings array

                        return [
                            "kodeMK" => $item->kodeMK,
                            "namaMK" => $item->namaMK,
                            "kodeBK" => $item->Bahan_Kajian
                                ->pluck("kodeBK")
                                ->implode(","),
                            "sks" => $item->sks,
                            "1" => $semesterCheck == 1 ? "✓" : "",
                ...
                {
                    $semesterHeadings = range(1, 8); // create an array of semester values from 1 to 8
                    $headings = ["Kode MK", "Nama MK", "Kode BK", "SKS"];
    ...
    public function exportToPDF(Request $request)
    {
        ...
        $pdf = PDF::loadView("content.pemetaan_susunan_mk.susunan_mk_pdf", [
            "data" => $data,
        ]);
