...
class RPSController extends Controller
{
    ...
    public function export($type, $idRPS, $kodeMK)
    {
        ...
        $kodePrasyaratList = Prasyarat::all()
            ->where("kodeMK", $kodeMK)
            ->pluck("mat_kodeMK")
            ->toArray();
        $prasyarat = Mata_Kuliah::whereIn("kodeMK", $kodePrasyaratList)
            ->distinct()
            ->get();
        $dosen = User::where("nip", $rps->nip)->first();
        $kodePengampu = Pengampu::all()
            ->where("id_rps", $rps->id_rps)
            ->pluck("nip")
            ->toArray();
        $pengampu = User::whereIn("nip", $kodePengampu)->distinct()->get();
        $kodeMingguRpsList = Minggu_RPS::all()
            ->where("id_rps", $idRPS)
            ->pluck("kodeMingguRPS")
            ->toArray();
        $kodeCPMKList = Detail_MK_CPMK::all()
            ...
            ->pluck("kodeCPMK")
            ->toArray();
        $kodeCPLList = CPMK::whereIn("kodeCPMK", $kodeCPMKList)
            ->get()
            ->pluck("kodeCPL")
            ->toArray();
        $list_cpl = CPL_Prodi::whereIn("kodeCPL", $kodeCPLList)
            ->distinct()
            ->get();
        ...
        $view = view("content.EksporRPS", [
            ...
            "cpl" => CPL_Prodi::all(),
            "kodeRPS" => $idRPS,
            "teknik_penilaian_list" => Teknik_Penilaian::where(
                "id_rps",
                $idRPS
            ),
            "detail_rps_list" => Detail_RPS_Penilaian::all(),
            "minggu_pustaka" => Detail_Pustaka_Minggurps::whereIn(
                "kodeMingguRPS",
                $kodeMingguRpsList
            )->get(),
            "list_prasyarat" => $prasyarat,
            "penanggung_jawab" => User::where(
                "nip",
                $rps->penanggungJawab
            )->first(),
    ...
    public function processData(Request $request)
    {
        ...
        if ($rps) {
            ...
            return view("content.cari_rps", [
                ...
                ->with("kodeRPS", $rps->kodeRPS)
                ->with("tahunAjaran", $rps->tahunAjaran);
        } else {
    ...
    public function store(Request $request)
    {
        ...
        $id_rps =
            $request->kodeMK .
            substr($request->tahunAjaran, -2) .
            str_pad($request->semester, 2, "0", STR_PAD_LEFT);
        ...
        return redirect()
            ->route("edit_rps.mata_kuliah", ["kodeRPS" => $id_rps])
            ->with([
                "success" => "Data RPS berhasil ditambahkan.",
                "kodeRPS" => $id_rps,
            ]);
    }
    ...
    {
        $newestYear = RPS::max("tahunAjaran");
        $newestSemester = RPS::where("tahunAjaran", $newestYear)->max(
            "semester"
        );
        $rps = RPS::where("tahunAjaran", $newestYear)
            ->where("semester", $newestSemester)
            ->get();
    ...
    public function detail($kodeRPS)
    {
        ...
        $semuaCPL = CPL_Prodi::whereIn("kodeCPL", $kodeCPLList)
            ->distinct()
            ->get();
        $kodePrasyaratList = Prasyarat::all()
            ->where("kodeMK", $kodeMK)
    ...
    public function export_filtered_excel($kodeMK)
    {
        return Excel::download(
            new ExportListFilteredRps($kodeMK),
            "list_rps.xlsx"
        );
    }
}
