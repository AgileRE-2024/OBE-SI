...
class CPLDiktiController extends Controller
{
    public function index()
    {
        ...
        return view("content.cpl_dikti.cpl_dikti", [
            "title" => "CPL Dikti",
            "cpls" => $cpls,
        ]);
    }
    ...
    {
        return view("content.cpl_dikti.add_cpl_dikti", [
    ...
    public function storeCPLDikti(Request $request)
    {
        ...
        return redirect()
            ->route("kurikulum.data.cpl_sndikti")
    ...
    public function edit($cpl)
    {
        ...
        return view("content.cpl_dikti.edit_cpl_dikti", [
            "title" => "Edit CPL SN Dikti",
            "cpl" => $cpl,
        ]);
    }
    ...
    public function update($cpl, Request $request)
    {
        ...
        if (
            CPL_SN_Dikti::where("kodeCPLSN", $request->kodeCPLSN)->exists() &&
            $request->kodeCPLSN != $cpl
        ) {
        ...
        return redirect()
            ->route("kurikulum.data.cpl_sndikti")
            ->with("success", "CPL SN Dikti berhasil diubah");
    }
    ...
    public function delete($cpl)
    {
        if (Detail_SN_CPLProdi::where("kodeCPLSN", $cpl)->exists()) {
            return redirect()
                ->route("kurikulum.data.cpl_sndikti")
                ->with("error", "CPL SN Dikti masih berelasi dengan CPL.");
        } else {
